        ...
        */
        ...
        private async void OnMarketData(Api api, MarketData marketData)
        {
            ...
            if (ticker.IndexOf("GFGV") > 0 || ticker.IndexOf("GGAL") > 0)
            {
                ...
                // Si lleg� un dato de GGAL, recalcular todos los PUTs
                if (ticker.Contains("GGAL"))
                {
                    RecalcularPUTs();
                }
        ...
        private async Task EjecutarArbitraje(string tickerPut, int cantidadContratos, decimal precioPut, decimal ratio)
        {
            ...
            try
            {
                // Extraer s�mbolo corto del PUT (ej: "MERV - XMEV - GFGV75029A - 24hs" -> "GFGV75029A")
                ...
                // Enviar ambas �rdenes en paralelo
                ...
                if (resPut.Exitosa && resGGAL.Exitosa)
                    ...
                    ToLog($"  >>> ATENCION: Revisar �rdenes manualmente");

                // Volver a obtener la liquidez despu�s de completar la compra
                await RefrescarLiquidez();
            }
            catch (Exception ex)
            {
